...
function tbs_register_menu() {
    ...
    add_submenu_page( 'trusted-by-svrice', 'General Settings', 'General',        'manage_options', 'trusted-by-svrice',               'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Hero Section',     'Hero Section',   'manage_options', 'trusted-by-svrice&tab=hero',       'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Fiverr Card',      'Fiverr Card',    'manage_options', 'trusted-by-svrice&tab=fcard',      'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Stats',            'Stats Section',  'manage_options', 'trusted-by-svrice&tab=stats',      'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Reviews',          'Client Reviews', 'manage_options', 'trusted-by-svrice&tab=reviews',    'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Testimonials',     'Testimonials',   'manage_options', 'trusted-by-svrice&tab=testimonials','tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'Tech & Services',  'Tech & Services','manage_options', 'trusted-by-svrice&tab=tech',       'tbs_render_admin_page' );
    add_submenu_page( 'trusted-by-svrice', 'CTA & Footer',     'CTA & Footer',   'manage_options', 'trusted-by-svrice&tab=cta',        'tbs_render_admin_page' );
}

add_action( 'admin_enqueue_scripts', 'tbs_admin_scripts' );
function tbs_admin_scripts( $hook ) {
    if ( strpos( $hook, 'trusted-by-svrice' ) === false ) return;
    ...
    <div class="tbs-wrap">
        <div class="tbs-header">
            ...
            <div class="tbs-shortcode-box">
                ...
                <code>[trust_by_svrice]</code>
                <button class="tbs-copy-btn" onclick="tbsCopy('[trust_by_svrice]', this)">📋 Copy</button>
            </div>
        </div>
        ...
        <div class="tbs-layout">
            <nav class="tbs-sidebar">
                <?php foreach ( $tabs as $key => $label ) :
                    $active  = ( $tab === $key || ( $tab === 'general' && $key === 'general' ) ) ? 'active' : '';
                    $url     = admin_url( 'admin.php?page=trusted-by-svrice&tab=' . $key );
                ?>
            ...
            <div class="tbs-main">
                <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>">
                    ...
                    <?php
                    switch ( $tab ) {
                        case 'hero':         tbs_tab_hero( $s );         break;
                        ...
                        default:             tbs_tab_general( $s );      break;
                    }
                    ?>
                    ...
                    <div class="tbs-save-bar">
                        ...
                        <?php if ( isset( $_GET['saved'] ) ) : ?>
                            <span class="tbs-saved-msg">✅ Settings saved successfully!</span>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
...
function tbs_card( $title, $content_cb ) {
    echo '<div class="tbs-card"><div class="tbs-card-title">' . $title . '</div>';
    $content_cb();
    echo '</div>';
}

function tbs_tab_general( $s ) {
    echo '<div class="tbs-tab-title">🏠 General Settings</div>';
    tbs_card( '🔗 Navigation', function() use ($s) {
        tbs_field( 'Logo Text',      'nav_logo_text',    $s['nav_logo_text'] );
        tbs_field( 'LinkedIn URL',   'nav_linkedin_url', $s['nav_linkedin_url'], 'url' );
        tbs_field( 'Twitter/X URL',  'nav_twitter_url',  $s['nav_twitter_url'],  'url' );
    });
}
...
function tbs_tab_hero( $s ) {
    ...
    tbs_card( '✏️ Hero Content', function() use ($s) {
        ...
        tbs_field( 'Rating Label',        'hero_rating_label', $s['hero_rating_label'] );
...
function tbs_tab_reviews( $s ) {
    ...
    foreach ( $reviews as $i => $r ) :
        ...
        tbs_card( "⭐ Review #{$n}", function() use ($r, $i) {
            ...
            tbs_field( 'Initial',       "reviews[{$i}][initial]", $r['initial'], 'text', 'Single letter shown in avatar' );
...
function tbs_tab_testimonials( $s ) {
    ...
    foreach ( $testis as $i => $t ) :
        ...
        tbs_card( "💬 Testimonial #{$n}", function() use ($t, $i) {
            ...
            tbs_field( 'Initial',       "testimonials[{$i}][initial]",  $t['initial'] );
            tbs_field( 'Avatar Color',  "testimonials[{$i}][color]",    $t['color'], 'color' );
            tbs_field( 'Text (HTML)',   "testimonials[{$i}][text]",     $t['text'],  'html' );
            tbs_field( 'Platform',      "testimonials[{$i}][platform]", $t['platform'] );
        });
    endforeach;
}
...
function tbs_tab_tech( $s ) {
    ...
    tbs_card( '⚙️ Technology Chips', function() use ($chips) {
        echo '<p class="tbs-desc">Each row = one chip shown on the page.</p>';
    ...
    tbs_card( '📋 Services', function() use ($services) {
        foreach ( $services as $i => $svc ) {
            ...
            echo '<input type="color" name="services[' . $i . '][bg]"   value="' . esc_attr($svc['bg'])    . '" style="width:50px;height:36px;border:none;padding:2px">';
...
function tbs_tab_cta( $s ) {
    echo '<div class="tbs-tab-title">🚀 CTA & Footer</div>';
    tbs_card( '🎯 Call to Action Section', function() use ($s) {
        tbs_field( 'Heading (HTML allowed)',  'cta_heading',   $s['cta_heading'],  'html' );
        tbs_field( 'Subtext',                 'cta_subtext',   $s['cta_subtext'],  'textarea' );
        tbs_field( 'Note Text',               'cta_note',      $s['cta_note'] );
    });
    ...
    tbs_card( '🔻 Footer', function() use ($s) {
        tbs_field( 'Footer Text', 'footer_text', $s['footer_text'] );
    });
}
