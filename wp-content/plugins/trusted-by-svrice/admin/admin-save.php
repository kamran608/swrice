...
function tbs_handle_save() {
    ...
    $tab     = isset( $_POST['tbs_tab'] ) ? sanitize_key( $_POST['tbs_tab'] ) : 'general';
    ...
    switch ( $tab ) {
        case 'general':
            $current['nav_logo_text']    = $str('nav_logo_text');
            $current['nav_linkedin_url'] = $url('nav_linkedin_url');
            $current['nav_twitter_url']  = $url('nav_twitter_url');
            break;
        ...
        case 'cta':
            ...
            $current['footer_text']   = $str('footer_text');
            break;
    }

    update_option( TBS_OPTION_KEY, $current );

    wp_redirect( admin_url( 'admin.php?page=trusted-by-svrice&tab=' . $tab . '&saved=1' ) );
    exit;
}
